...
- Os títulos de Débito e Crédito mostram duas linhas quando o ciclo selecionado já começou: **Hoje** é um retrato independente dos filtros, usando somente registros reais `pago = true` com `data` até a data local atual; **Futuro** mantém a projeção completa do ciclo, com pagos e abertos. Ao navegar para um ciclo ainda não iniciado, ambos mostram somente Futuro. A prévia de Crédito do ciclo atual mantém Hoje mesmo que sua fatura vença no próximo ciclo. No Débito, Saldo começa em `SALDO_DESDE` com saldo inicial zero e soma os débitos reais já ocorridos (inclui aporte/resgate, pois o dinheiro saiu ou voltou à conta); Guardado é essa parcela de Investimento separada. Registros anteriores a `SALDO_DESDE` e sugestões sintéticas nunca entram em Hoje. No Crédito, Hoje é a fatura líquida das compras confirmadas e antecipações já ocorridas, enquanto Futuro mantém a fatura líquida de compras pagas e abertas.
- O único cartão atual é o Nubank. Todo `cred = true` pertence a ele enquanto não existir modelo explícito para outro cartão; nomes e categorias não identificam cartão.
- O total no título dessa prévia de Crédito usa os mesmos filtros e abatimentos por antecipação da linha dinâmica da fatura; a tabela abaixo continua detalhando as compras brutas. O limite livre do Nubank ignora filtros visuais para retratar o cartão real: crédito com `pago = false` é projeção e não ocupa limite; crédito com `pago = true` ocupa; antecipação alocada à mesma fatura libera somente o que ela abate, até zero.
- `LIMITE_CARTAO` é o limite contratado-base do Nubank. Ele aparece como campo discreto no próprio “de R$”, com a mesma máscara monetária do cadastro; Enter ou sair dele salva a preferência somente neste navegador, sem gravar no Supabase. O teto utilizável soma a esse limite apenas a garantia positiva: aportes e resgates reais de `Investimento` até o ciclo, independente do filtro Pago ou de qualquer filtro visual. `Aporte sugerido` não é dinheiro aplicado e não entra na garantia; resgate reduz a garantia. Garantia negativa não reduz o limite contratado e a parcela positiva aparece ao lado quando existir. O título também exibe o **Total**, que é exatamente limite contratado-base mais garantia positiva.
- Um segundo cartão só poderá ser criado com uma decisão própria e migration nova: cada lançamento de crédito e antecipação deverá ter uma identificação explícita de cartão, e cada cartão deverá ter seu limite contratado, garantia, faturas e alocação de antecipações independentes. O formulário, tabelas, gráficos, filtros, cálculos e testes precisarão respeitar essa separação; não reutilizar `LIMITE_CARTAO`, nem deduzir cartão por nome, categoria ou vencimento.
