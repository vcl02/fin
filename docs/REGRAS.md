...
- Os títulos de Débito e Crédito mostram duas linhas quando a competência já começou: **Hoje** é um retrato independente dos filtros, usando somente registros reais `pago = true` com `data` até a data local atual; **Futuro** mantém a projeção completa do ciclo, com pagos e abertos. Ciclos e faturas cuja competência ainda não começou mostram somente Futuro, pois não possuem retrato de Hoje. No Débito, Saldo começa em `SALDO_DESDE` com saldo inicial zero e soma os débitos reais já ocorridos (inclui aporte/resgate, pois o dinheiro saiu ou voltou à conta); Guardado é essa parcela de Investimento separada. Registros anteriores a `SALDO_DESDE` e sugestões sintéticas nunca entram em Hoje. No Crédito, Hoje é a fatura líquida das compras confirmadas e antecipações já ocorridas, enquanto Futuro mantém a fatura líquida de compras pagas e abertas.
