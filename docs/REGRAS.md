...
- As regras de consistência ficam centralizadas em `validarLancamentosCarregados()` em `js/domain.js`. Para criar uma regra personalizada, acrescente ali a condição e uma mensagem em `avisos`; o botão e o modal passam a refletir o resultado sem mudar o banco. Categoria preenchida que aparece em somente um lançamento carregado gera aviso suspeito; diferenças apenas de espaço, caixa ou acento contam como a mesma categoria.
