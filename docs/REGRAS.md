...
- Os títulos de Débito e Crédito mostram duas linhas: **Hoje** é um retrato independente dos filtros, usando somente registros reais `pago = true` com `data` até a data local atual; **Futuro** mantém a projeção completa do ciclo, com pagos e abertos. No Débito, Saldo é a soma dos débitos reais já ocorridos (inclui aporte/resgate, pois o dinheiro saiu ou voltou à conta) e Guardado é essa parcela de Investimento separada; sugestões sintéticas nunca entram em Hoje. No Crédito, Hoje é a fatura líquida das compras confirmadas e antecipações já ocorridas, enquanto Futuro mantém a fatura líquida de compras pagas e abertas.
