...
- `LIMITE_CARTAO` é o limite contratado-base do Nubank. Ele aparece como campo discreto no próprio “de R$”; Enter ou sair dele salva a preferência somente neste navegador, sem gravar no Supabase. O teto utilizável soma a esse limite apenas o `guardadoAte(ciclo)` positivo, exatamente o valor do título Débito aplicado como limite garantido Nubank; guardado negativo não reduz o limite contratado e a garantia aparece ao lado quando existir.
