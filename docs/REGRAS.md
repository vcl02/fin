...
- O total no título dessa prévia de Crédito usa os mesmos filtros e abatimentos por antecipação da linha dinâmica da fatura; a tabela abaixo continua detalhando as compras brutas. O mesmo título mostra o limite livre do único cartão, definido em `LIMITE_CARTAO` em `js/domain.js`: crédito com `pago = false` é projeção e não compromete o limite; crédito com `pago = true` compromete, e a antecipação alocada à fatura o libera até zero. O limite ignora filtros visuais para sempre retratar o cartão real. O limite contratado aparece como campo discreto no próprio “de R$”; Enter ou sair dele salva a preferência somente neste navegador, sem gravar no Supabase. O teto exibido soma a esse limite apenas o `guardadoAte(ciclo)` positivo, exatamente o valor do título Débito daquele ciclo aplicado como limite garantido; guardado negativo não reduz o limite contratado e a garantia aparece ao lado quando existir.
