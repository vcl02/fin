...
test('soma nove ciclos previstos e acompanha o aumento da Evolução Obra', () => {
    const d = calcular([
        { periodoIdx: 4, nome: 'Condomínio', v: -100, reserva: true },
        { periodoIdx: 4, nome: 'Evolução Obra', v: -200, reserva: true },
        { periodoIdx: 5, nome: 'Evolução Obra', v: -300, reserva: true },
        { periodoIdx: 6, nome: 'Evolução Obra', v: -400, reserva: true },
        { periodoIdx: 5, nome: 'Outros', v: -700, reserva: false },
        { periodoIdx: 5, nome: 'Antecipação', v: -800, reserva: true, _transferencia: true },
        { periodoIdx: 5, nome: 'Estorno', v: 50, reserva: true },
    ], 4, 450);
...
test('gasto que começa no futuro entra na meta e valor sem ocorrência é mantido', () => {
    const d = calcular([
        { periodoIdx: 2, nome: 'Casa', v: -100, reserva: true },
        { periodoIdx: 3, nome: 'Seguro', v: -50, reserva: true },
    ], 2, 0);
...
test('progresso fica limitado a cem por cento e mostra excedente', () => {
    const d = calcular([{ periodoIdx: 1, nome: 'Casa', v: -100, reserva: true }], 1, 1200);
...
test('sem gastos marcados a meta e o progresso ficam zerados', () => {
    const d = calcular([{ periodoIdx: 1, nome: 'Casa', v: -100, reserva: false }], 1, 500);
...
test('soma lançamentos do mesmo nome no mesmo ciclo antes de projetar', () => {
    const d = calcular([
        { periodoIdx: 3, nome: 'Casa', v: -100, reserva: true },
        { periodoIdx: 3, nome: 'Casa', v: -50, reserva: true },
    ], 3, 0);
...
test('não inclui entradas, transferências nem classificação textual no cálculo', () => {
    const d = calcular([
        { periodoIdx: 7, nome: 'Entrada', v: 500, reserva: true },
        { periodoIdx: 7, nome: 'Transferência', v: -400, reserva: true, _transferencia: true },
        { periodoIdx: 7, nome: 'Texto', v: -300, reserva: 'true' },
        { periodoIdx: 7, nome: 'Válido', v: -200, reserva: true },
    ], 7, -100);
